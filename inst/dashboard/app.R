...
df <- readxl::read_excel("resultado_cpc_2016_portal_27_11_2017.xlsx") %>% 
  janitor::clean_names() %>% # retira acento, espaco em branco e converte para caixa-baixa
  dplyr::select(-c(
    ano, codigo_do_curso, codigo_da_area, 
    codigo_do_municipio, concluintes_inscritos,
  ...
  dplyr::select_all(~stringr::str_replace(., "_padronizada", "")) %>% # retira o termo padronizada do nome
  dplyr::rename(
  ...
  dplyr::filter(nr_de_docentes < 150) # filtra numero absurso de professores
  #dplyr::mutate(id_filtro = dplyr::row_number())
